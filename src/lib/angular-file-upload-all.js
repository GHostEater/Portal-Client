...
angularFileUpload.directive('ngFileSelect', ['$parse', '$timeout', '$compile',
    function ($parse, $timeout, $compile) {
        return {
            restrict: 'AEC',
            require: '?ngvm',
            link: function (scope, elem, attr, ngvm) {
                linkFileSelect(scope, elem, attr, ngvm, $parse, $timeout, $compile);
            }
        }
    }]);

function linkFileSelect(scope, elem, attr, ngvm, $parse, $timeout, $compile) {
    ...
    function changeFn(evt) {
        if (!isUpdating) {
            ...
            try {
                ...
                updatevm($parse, $timeout, scope, ngvm, attr,
    ...
    if (window.FileAPI && window.FileAPI.ngfFixIE) {
        window.FileAPI.ngfFixIE(elem, createFileInput, changeFn, resetvm);
    } else {
        elem.bind('click', clickHandler);
    }
}
...
angularFileUpload.directive('ngFileDrop', ['$parse', '$timeout', '$location', function ($parse, $timeout, $location) {
    return {
        ...
        require: '?ngvm',
        link: function (scope, elem, attr, ngvm) {
            linkDrop(scope, elem, attr, ngvm, $parse, $timeout, $location);
        }
    }
}]);
...
function linkDrop(scope, elem, attr, ngvm, $parse, $timeout, $location) {
...
function updatevm($parse, $timeout, scope, ngvm, attr, fileChange, files, rejFiles, evt, noDelay) {
    function update() {
        if (ngvm) {
            $parse(attr.ngvm).assign(scope, files);
            $timeout(function () {
                ngvm && ngvm.$setViewValue(files != null && files.length == 0 ? null : files);
            });
        }
        if (attr.ngvmRejected) {
            $parse(attr.ngvmRejected).assign(scope, rejFiles);
        }
        if (fileChange) {
...
if ((window.XMLHttpRequest && !window.FormData) || (window.FileAPI && FileAPI.forceLoad)) {
	...
	FileAPI.ngfFixIE = function(elem, createFileElemFn, changeFn, resetvm) {
		if (!hasFlash()) {
			throw 'Adode Flash Player need to be installed. To check ahead use "FileAPI.hasFlash"';
		}
